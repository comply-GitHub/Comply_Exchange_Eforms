    ...
    return (<>
        {
            error.message !== "" && error.message !== null && isErrorVisible ?
                <div >
                    <Alert severity="error" sx={{ height: "5vh", display: "grid", gridTemplateColumns: "2% 95%" }} >
                        <div
                            style={{ display: "grid", gridTemplateColumns: "95% 5%" }}
                        >
                            <Typography>{error.message} </Typography><Typography sx={{ color: "black", justifySelf: "end" }} onClick={() => setIsErrorVisible(false)}>X</Typography>
                        </div>
                    </Alert>
